...
Each monkey has several attributes:
        ...
        item. (An operation like new = old * 5 means that your worry level
...
up inspecting and throwing many items by the time its turn comes around. If
...
class Monkey(object):
    ...
    def process(self):
        ...
        while len(items) > 0:
            ...
            #print("\tMonkey inspects an item with a worry level of {}.".format(head))
            worry = self._inspect(head)
            #print("\t\tWorry level {} to {}".format(self.operation, worry))
            worry //= 3
            #print("\t\tMonkey gets bored with item. "
            #      "Worry level is divided by 3 to {}.".format(worry))
            destination = self.test(worry)
            movements.append((destination, worry))
        self.items = items
        return movements



monkeys = get_monkeys("input.json")
for _ in range(20):
    for monkey in monkeys:
        mvs = monkey.process()
        for dest, item in mvs:
            #print("\t\tItem with worry level {} is thrown to monkey {}.".format(
            #    item,  dest))
